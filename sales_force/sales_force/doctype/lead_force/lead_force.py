...
class LeadForce(SellingController, CRMNote):
	def get_feed(self):
		...
		if self.email_id:
			...
			if not frappe.db.get_single_value("CRM Settings", "allow_lead_duplication_based_on_emails"):
				duplicate_leads = frappe.get_all(
					"Lead Force", filters={"email_id": self.email_id, "name": ["!=", self.name]}
				)
				duplicate_leads = [
					frappe.bold(get_link_to_form("Lead Force", lead.name)) for lead in duplicate_leads
				]
	...
	def link_to_contact(self):
		...
		if self.contact_doc:
			self.contact_doc.append(
				"links", {"link_doctype": "Lead Force", "link_name": self.name, "link_title": self.lead_name}
			)
...
def make_quotation(source_name, target_doc=None):
	...
	target_doc = get_mapped_doc(
		"Lead Force",
		source_name,
		{"Lead Force": {"doctype": "Quotation", "field_map": {"name": "party_name"}}},
		target_doc,
		set_missing_values,
	)

	target_doc.quotation_to = "Lead Force"
...
def make_lead_from_communication(communication, ignore_communication_links=False):
	...
	lead_name = None
	if doc.sender:
		lead_name = frappe.db.get_value("Lead Force", {"email_id": doc.sender})
	if not lead_name and doc.phone_no:
		lead_name = frappe.db.get_value("Lead Force", {"mobile_no": doc.phone_no})
	if not lead_name:
		lead = frappe.get_doc(
			{
				"doctype": "Lead Force",
